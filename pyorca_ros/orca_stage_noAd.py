...
if __name__ == "__main__":
    ...
    #init orca node
    os.environ["ROS_MASTER_URI"]="http://localhost:%d"%(ROSPORT)
    rospy.init_node("test_orca_%d"%rank, anonymous=None)
    reset_stage = rospy.ServiceProxy('reset_positions', Empty)
    reset_stage()

    #init orca agents
    env0= Agent(robo_index = 0, radius = 0.3,max_speed = 1,goal=[0,4],env_index = 3)
    env1= Agent(robo_index = 1, radius = 0.3,max_speed = 1,goal=[0,4],env_index = 3)
    env2= Agent(robo_index = 2, radius = 0.3,max_speed = 1,goal=[0,4],env_index = 3)
    env3= Agent(robo_index = 3, radius = 0.3,max_speed = 1,goal=[0,4],env_index = 3)
    env4= Agent(robo_index = 4, radius = 0.3,max_speed = 1,goal=[0,4],env_index = 3)
    env5= Agent(robo_index = 5, radius = 0.3,max_speed = 1,goal=[0,4],env_index = 3)
    agents = [env0,env1,env2,env3,env4,env5]
    # env = env0
    env = agents[rank]
    ...
    for id in range(5000):
        ...
        while not terminate:
            ...
            agents[rank].control_vel(new_vels[rank])
