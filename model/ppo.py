...
def generate_action_adChaseV2(env, state_list, policy, action_bound):
    if env.mpi_rank == 0:
        s_list, goal_list, speed_list = [], [], []
        for i in state_list:
            goal_list.append(i[0])
...
def transform_buffer_adChaseV2(buff):
    goal_batch, speed_batch, a_batch, r_batch, d_batch, l_batch, \
    v_batch = [], [], [], [], [], [], []
    s_temp, goal_temp, speed_temp = [], [], []
...
def ppo_update_adversayChaseV2(policy, optimizer, batch_size, memory, epoch,
               coeff_entropy=0.02, clip_value=0.2,
               num_step=2048, num_env=12, frames=1, obs_size=24, act_size=4):
    # obss, goals, speeds, actions, logprobs, targets, values, rewards, advs = memory
    goals, speeds, actions, logprobs, targets, values, rewards, advs = memory
    ...
    # obss = obss.reshape((num_step*num_env, frames, obs_size))
    goals = goals.reshape((num_step*num_env, 8))
    speeds = speeds.reshape((num_step*num_env, 8))
    actions = actions.reshape(num_step*num_env, act_size)
    logprobs = logprobs.reshape(num_step*num_env, 1)
    advs = advs.reshape(num_step*num_env, 1)
    targets = targets.reshape(num_step*num_env, 1)
...
def generate_action_adGeneralV2(env, state_list, policy, action_bound):
    if env.mpi_rank == 0:
        s_list, goal_list, speed_list = [], [], []
        for i in state_list:
            goal_list.append(i[0])
        ...
        # v, a, logprob, mean = policy(s_list, goal_list, speed_list)
        v, a, logprob, mean = policy(goal_list)
        v, a, logprob = v.data.cpu().numpy(), a.data.cpu().numpy(), logprob.data.cpu().numpy()
        scaled_action = np.clip(a, a_min=action_bound[0], a_max=action_bound[1])
    else:
        v = None
...
def transform_buffer_adGeneralV2(buff):
    goal_batch, a_batch, r_batch, d_batch, l_batch, \
    v_batch = [], [], [], [], [], []
    s_temp, goal_temp = [], []
    ...
    return goal_batch, a_batch, r_batch, d_batch, l_batch, v_batch

def ppo_update_adversayGeneralV2(policy, optimizer, batch_size, memory, epoch,
               coeff_entropy=0.02, clip_value=0.2,
               num_step=2048, num_env=12, frames=1, obs_size=24, act_size=4):
    # obss, goals, speeds, actions, logprobs, targets, values, rewards, advs = memory
    goals, actions, logprobs, targets, values, rewards, advs = memory
    ...
    for update in range(epoch):
        ...
        for i, index in enumerate(sampler):
            ...
            # new_value, new_logprob, dist_entropy = policy.evaluate_actions(sampled_obs, sampled_goals, sampled_speeds, sampled_actions)
            new_value, new_logprob, dist_entropy = policy.evaluate_actions(sampled_goals, sampled_actions)
