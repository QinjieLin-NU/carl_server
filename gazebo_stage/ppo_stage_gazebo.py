...
if __name__ == '__main__':
    ROS_PORT0 = 11412#11312 we let env_index = 1
    NUM_BOT = 24#8
    NUM_ENV = 25#48
    ID = 34#30
    Train = True #True
    ...
    env = None
    if(rosPort == 11412):
        envIndex =  int(rank/NUM_BOT) + StageEnvStart
        env = StageWorld(beam_num=360, index=robotIndex, num_env=NUM_ENV,ros_port = rosPort,mpi_rank = rank,env_index = envIndex)
    else:
        envIndex = gazeboEnvIndex
        env = GazeboWorld(beam_num=360, index=robotIndex, num_env=NUM_ENV,ros_port = rosPort,mpi_rank = rank,env_index = gazeboEnvIndex)
    reward = None
